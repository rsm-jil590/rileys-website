...
# Base curve
ax.plot(x, y, color='steelblue', linewidth=2, label='$N(0, 1)$ under $H_0$')

# Rejection regions at alpha = 0.05 (|z| > 1.96)
z_crit = 1.96
x_left  = x[x <= -z_crit]
x_right = x[x >=  z_crit]
ax.fill_between(x_left,  stats.norm.pdf(x_left),  color='crimson', alpha=0.25,
                label=r'Rejection region ($\alpha = 0.05$)')
ax.fill_between(x_right, stats.norm.pdf(x_right), color='crimson', alpha=0.25)

# P-value tails for observed z
z_obs = z_stat
x_p_right = x[x >=  abs(z_obs)]
x_p_left  = x[x <= -abs(z_obs)]
ax.fill_between(x_p_right, stats.norm.pdf(x_p_right), color='orange', alpha=0.6,
                label=f'p-value tails (p = {p_value:.3f})')
ax.fill_between(x_p_left,  stats.norm.pdf(x_p_left),  color='orange', alpha=0.6)

# Observed z marker
ax.axvline(z_obs, color='darkgreen', linestyle='--', linewidth=2,
           label=f'Observed $z$ = {z_obs:.2f}')

# Critical value markers
ax.axvline( z_crit, color='crimson', linestyle=':', linewidth=1.2, alpha=0.7)
ax.axvline(-z_crit, color='crimson', linestyle=':', linewidth=1.2, alpha=0.7)
